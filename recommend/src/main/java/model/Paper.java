...
public class Paper implements Writable {
	...
	public int similarity(PaperRecommendQueryEntity queryEntity) {
		...
		similarity += SimilarityUtil.stringSimilarity(this.author, queryEntity.authers) * 3;
		similarity += SimilarityUtil.stringSimilarity(this.title, queryEntity.titleKeywords) * 2;
	...
	public void fillIn(Put put) {
		try {
			...
			put.addColumn("paper".getBytes(CHARSET), "pages".getBytes(CHARSET), pages.getBytes(CHARSET));
